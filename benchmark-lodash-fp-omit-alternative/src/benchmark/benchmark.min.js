(function (window, undefined) {
    ...
    var perfName =
      perfObject &&
      ((perfObject.now && "now") || (perfObject.webkitNow && "webkitNow"));
    ...
    var calledBy = {};
    var divisors = { 1: 4096, 2: 512, 3: 64, 4: 8, 5: 0 };
    var tTable = {
      1: 12.706,
      2: 4.303,
      3: 3.182,
    ...
    (function () {
      ...
      try {
        var o = {};
        support.descriptors =
          (setDescriptor(o, o, o), "value" in getDescriptor(o, o));
      } catch (e) {
      ...
      try {
        support.nodeClass =
          ({ toString: 0 } + "", toString.call(doc || 0) != "[object Object]");
      } catch (e) {
        support.nodeClass = true;
      }
    })();
    var timer = { ns: Date, start: null, stop: null };
    ...
    function forProps() {
      ...
      forProps = function (object, callback, options) {
        ...
        if (allFlag && support.getAllKeys) {
          for (
            index = 0, keys = getAllKeys(object), length = keys.length;
            index < length;
            index++
          ) {
            key = keys[index];
          ...
          if (!done && forShadowed) {
            ...
            skipCtor =
              ctor && ctor.prototype && ctor.prototype.constructor === ctor;
    ...
    function isPlainObject(value) {
      ...
      if (
        ...
        (noArgumentsClass && isArguments(value))
      ) {
        return result;
      }
      ...
      do {
        ...
        if (value === Object(value)) {
          ...
          if (
            ...
            clone != value &&
            !((descriptor =
              source && support.descriptors && getDescriptor(source, key)),
            ...
            if ((extensible = isExtensible(value))) {
              ...
              for (
                subIndex = 0, length = unmarked.length;
                subIndex < length;
                subIndex++
              ) {
    ...
    function invoke(benches, name) {
      ...
      function isAsync(object) {
        ...
        return (
          Object(object).constructor == Benchmark &&
          name == "run" &&
    ...
    function cloneSuite(options) {
      ...
      forOwn(me, function (value, key) {
        if (!hasKey(result, key)) {
          result[key] =
            value && isClassOf(value.clone, "Function")
              ? value.clone()
              : deepClone(value);
        }
      });
    ...
    function emit(type) {
      ...
      if (
        events &&
        (listeners = hasKey(events, event.type) && events[event.type])
      ) {
    ...
    function off(type, listener) {
      var me = this,
        events = me.events;
      events &&
        each(type ? type.split(" ") : events, function (listeners, type) {
          var index;
    ...
    function on(type, listener) {
      ...
      forEach(type.split(" "), function (type) {
        (hasKey(events, type) ? events[type] : (events[type] = [])).push(
          listener
        );
      });
      return me;
        ...
        do {
          forOwn(data.source, function (value, key) {
            ...
            if (value && typeof value == "object") {
              ...
              value !== currValue &&
              !(value == null || isClassOf(value, "Function"))
            ) {
              changes[changes.length++] = {
                destination: destination,
                key: key,
    ...
    function clock() {
        ...
        while (count--) {
          if (unit == "us") {
            ...
            if (ns.nanoTime) {
              ...
              while (
                !(measured = (measured = ns())[0] + measured[1] / divisor - begin)
              ) {}
              divisor = 1;
            }
          } else {
      ...
      if (timer.unit == "ns") {
        ...
        if (timer.ns.stop) {
          ...
          extend(template, { begin: "s$=n$()", end: "r$=(n$()-s$)/1e6" });
        }
      }
    ...
    function run(options) {
      ...
      if (!event.cancelled) {
        options = {
          async:
            ((options = options && options.async) == null ? me.async : options) &&
            support.timeout
        };
        if (me._original) {
          if (me.defer) {
    ...
    extend(Benchmark, { Deferred: Deferred, Event: Event, Suite: Suite });
    if (
      typeof define == "function" &&
      typeof define.amd == "object" &&
      define.amd
